  ...
  return (
    <div
      style={{
        ...
        borderRadius: 18,
        overflow: "hidden",
        border: "1px solid rgba(150,40,200,0.2)",
        boxShadow: "0 8px 32px rgba(100,20,160,0.2)",
        transition: "transform 0.3s ease, box-shadow 0.3s ease, border-color 0.3s ease",
        cursor: "pointer",
      }}
      onMouseEnter={e => {
        e.currentTarget.style.transform = "translateY(-8px)";
        e.currentTarget.style.boxShadow = "0 20px 60px rgba(150,40,200,0.45)";
        e.currentTarget.style.borderColor = "rgba(180,79,255,0.6)";
      }}
      onMouseLeave={e => {
        e.currentTarget.style.transform = "translateY(0)";
        e.currentTarget.style.boxShadow = "0 8px 32px rgba(100,20,160,0.2)";
        e.currentTarget.style.borderColor = "rgba(150,40,200,0.2)";
      }}
    >
      <img src={`/${file}`} alt="DCA Bot Result" style={{ width: "100%", height: "auto", display: "block" }} />
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div style={{ position: "relative", zIndex: 1, paddingTop: 130, paddingBottom: 100 }}>
        <div className="page-wrap" style={{ maxWidth: 1300, margin: "0 auto", padding: "0 48px" }}>
          ...
          <div className="dca-grid">
            {DCA_CARDS.map((file, i) => (
              <DcaCard key={i} file={file} />
            ))}
          </div>
        </div>
      </div>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "28px 48px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", fontFamily: "'Space Mono', monospace" }}>© 2026 · Powered by Pangeon DEX</div>
        <a href="https://pangeon.xyz" style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", textDecoration: "none" }}>Pangeon DEX ↗</a>
      </footer>
    </>
  );
}
