...
function TradeCarousel() {
  ...
  const getStyle = (i) => {
    ...
    if (Math.abs(normalizedDiff) === 1) return {
      transform: `translateX(${normalizedDiff * 65}%) scale(0.78) rotateY(${-normalizedDiff * 25}deg)`,
      zIndex: 8, opacity: 0.7, filter: "brightness(0.6)",
    };
    if (Math.abs(normalizedDiff) === 2) return {
      transform: `translateX(${normalizedDiff * 75}%) scale(0.58) rotateY(${-normalizedDiff * 30}deg)`,
      zIndex: 6, opacity: 0.4, filter: "brightness(0.4)",
    };
    return {
      transform: `translateX(${normalizedDiff * 80}%) scale(0.4)`,
      zIndex: 1, opacity: 0, filter: "brightness(0.2)",
    };
  };

  return (
    <div
      style={{ position: "relative", width: "100%", height: 340, perspective: "1200px", cursor: "grab" }}
    ...
    >
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRadius: 16,
                overflow: "hidden",
                border: i === current ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(255,255,255,0.07)",
                cursor: i === current ? "default" : "pointer",
                transition: "all 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)",
                boxShadow: i === current ? "0 24px 80px rgba(0,0,0,0.8)" : "none",
                ...style,
              }}
            >
              <img
              ...
              />
              {i === current && (
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "16px 20px", background: "linear-gradient(transparent, rgba(0,0,0,0.85))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 14, fontWeight: 600, color: "#fff" }}>{t.token}</span>
                  <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", letterSpacing: "1px" }}>LIVE TRADE</span>
                </div>
              )}
            </div>
          );
      ...
      <button
        onClick={prev}
        style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", width: 40, height: 40, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.15)", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 16, cursor: "pointer", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(10px)", transition: "all 0.2s" }}
        onMouseEnter={e => e.target.style.background = "rgba(255,255,255,0.15)"}
        onMouseLeave={e => e.target.style.background = "rgba(0,0,0,0.6)"}
      >←</button>
      <button
        onClick={next}
        style={{ position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)", width: 40, height: 40, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.15)", background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 16, cursor: "pointer", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(10px)", transition: "all 0.2s" }}
        onMouseEnter={e => e.target.style.background = "rgba(255,255,255,0.15)"}
        onMouseLeave={e => e.target.style.background = "rgba(0,0,0,0.6)"}
      >→</button>
      ...
      <div style={{ position: "absolute", bottom: -28, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 6 }}>
        {TRADE_IMAGES.map((_, i) => (
          <div
            key={i}
            onClick={() => setCurrent(i)}
            style={{ width: i === current ? 20 : 6, height: 6, borderRadius: 3, background: i === current ? "#fff" : "rgba(255,255,255,0.2)", cursor: "pointer", transition: "all 0.3s" }}
          />
        ))}
      </div>
    </div>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .bot-section { padding: 120px 48px 80px; max-width: 1200px; margin: 0 auto; }
      ...
      <section className="bot-section">
        ...
        <div className="coming-soon-banner">
          ...
          <div className="coming-soon-desc">
            Automated trading bot with zone-based entries, smart exits and capital protection — launching Q4 2026.
          </div>
        </div>
      </section>
{/* BUILT BY */}
